...
- While the agent is working there will not be any console output. Set a timeout of at least 5 minutes and up to 30 minutes for complex tasks. There will be no console output until the agent is done. Do not prematurely stop the agent. Wait for the agent to finish.
...
- **Reviews** (combined reviewer, final reviewer): You are the reviewer. Review the code that the agent produced. Use `reviewer-prompt.md` for combined spec compliance + code quality reviews.