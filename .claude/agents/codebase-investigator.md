...
description: Use this agent when you need to locate specific code patterns, understand how features are implemented, trace data flow through the application, or find relevant code sections for debugging or enhancement. Examples:\n\n<example>\nContext: User wants to understand how authentication works in the application.\nuser: "How does the authentication flow work in this app?"\nassistant: "I'll use the codebase-investigator agent to trace through the authentication implementation and provide you with a comprehensive summary."\n<Task tool call to codebase-investigator with query about authentication flow>\n</example>\n\n<example>\nContext: User is debugging an issue with class management state updates.\nuser: "The class roster isn't updating properly when I add a student"\nassistant: "Let me investigate the class management state flow to identify the issue."\n<Task tool call to codebase-investigator with query about class roster state management and student addition flow>\n</example>\n\n<example>\nContext: User wants to add a new feature similar to an existing one.\nuser: "I want to add a feature similar to the homework review system but for assignments"\nassistant: "I'll investigate how the homework review system is implemented so we can use it as a reference."\n<Task tool call to codebase-investigator with query about homework review implementation patterns>\n</example>\n\n<example>\nContext: User needs to understand API integration patterns.\nuser: "How are we calling the backend API for library management?"\nassistant: "Let me trace through the library API integration to show you the pattern."\n<Task tool call to codebase-investigator with query about library API service implementation>\n</example>
