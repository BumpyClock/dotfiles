...
async function createTemporaryDirectory(): Promise<string> {
	const directory = await mkdtemp(
		path.join(os.tmpdir(), "setup-ai-agents-test-"),
	);
...
describe("ai-agent-links config", () => {
	...
	test("mirrors generated agent directories by replacing stale files", async () => {
		...
		await writeFile(
			path.join(sourcePath, "nested", "developer.md"),
			"developer-v1",
		);
		...
		expect(await readFile(path.join(targetPath, "architect.md"), "utf8")).toBe(
			"architect-v1",
		);
		expect(
			await readFile(path.join(targetPath, "nested", "developer.md"), "utf8"),
		).toBe("developer-v1");
		...
		expect(await readFile(path.join(targetPath, "reviewer.md"), "utf8")).toBe(
			"reviewer-v1",
		);
	...
	test("force replace refreshes mirrored directories even when contents already match", async () => {
		...
		expect(
			(await readdir(root)).filter((entry) =>
				entry.startsWith("target.backup."),
			),
		).toEqual([]);
	});
});
