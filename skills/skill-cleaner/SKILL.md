...
- It scans `~/.codex/history.jsonl`, ~/.pi, ~/.claude, ~/.copilot, and ~/.opencode and recent `~/.codex/sessions/**/*.jsonl` by default. Add `--deep-logs` . fan out parallel sub-agents for deep and time efficient investigations.
- Usage evidence is heuristic: `$skill`, `Use $skill`, and paths like `skills/<name>/SKILL.md` and when there's tool calls that indicate that a skill was invoked. 
